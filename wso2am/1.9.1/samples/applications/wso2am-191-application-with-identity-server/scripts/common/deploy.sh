...
curl -X POST -H "Content-Type: application/json" -d "@${iaas_cartridges_path}/wso2is-500-manager.json" -k -v -u admin:admin https://${host_ip}:${host_port}/api/cartridges
...
echo "Adding WSO2 ${product_type} - ${product_version} cartridge Group 3 ..."
curl -X POST -H "Content-Type: application/json" -d "@${cartridges_groups_path}/${product}-group3.json" -k -v -u admin:admin https://${host_ip}:${host_port}/api/cartridgeGroups
...
curl -X POST -H "Content-Type: application/json" -k -v -u admin:admin https://${host_ip}:${host_port}/api/applications/${product}-application-with-is/deploy/${application_policy_id}
