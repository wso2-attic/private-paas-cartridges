...
echo "Adding WSO2 ${product_type} - ${product_version} Presenter cartridge..."
curl -X POST -H "Content-Type: application/json" -d "@${iaas_cartridges_path}/${product}-presenter.json" -k -v -u admin:admin https://${host_ip}:${host_port}/api/cartridges
